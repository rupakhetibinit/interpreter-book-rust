...
use crate::{
    ...
    lexer::{self, lexer::Token, precedence::Precedence, token::TokenType},
};
...
impl<'a> Parser<'a> {
    ...
    pub fn next_token(&mut self) -> Option<Result<Token<'a>, String>> {
        self.lexer.next()
    }
    ...
    fn parse_statement(&mut self) -> Option<Statement<'_>> {
        match self.lexer.next().is_some() {
